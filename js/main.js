...
const setup = () => {
    ...
    for (let i = 0; i < 5; i++) {
        platforms.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height })
    }
}
...
const update = () => {
    ...
    drawRect(position.x, position.y, 100, 100);
    ...
    platforms.forEach((platform, i) => {
        ...
        if ((position.y + 100 >= platform.y && position.y -- 100 <= platform.y)  && position.x + 100 >= platform.x && position.x < platform.x + 100) {
            velocity.y = 0
